...
class EditProductOutScreen extends StatefulWidget {
  final int? id;
  const EditProductOutScreen({Key? key, this.id}) : super(key: key);

  @override
  State<EditProductOutScreen> createState() => _EditProductOutScreenState();
}

class _EditProductOutScreenState extends State<EditProductOutScreen> {
  ...
  Future putProduct(
    ...
    try {
      final response = await dio.put("${MyColors.baseUrl}/api/products-out/${widget.id}",
          data: data,
  ...
  Future<List<ProductModel>> fetchProductData() async {
    final dio = Dio();
    String url = "${MyColors.baseUrl}/api/products-out/${widget.id}";

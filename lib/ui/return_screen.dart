...
import 'package:inventory/models/return_model.dart';
import 'package:inventory/ui/add_return_screen.dart';
import 'package:inventory/ui/edit_return_screen.dart';
...
class ReturnScreen extends StatefulWidget {
  ...
  const ReturnScreen({Key? key, this.id, this.nama}) : super(key: key);
...
class _ReturnScreenState extends State<ReturnScreen> {
  late Future<List<ReturnModel>> _productFuture;
  ...
  Future<List<ReturnModel>> fetchProductData() async {
    final dio = Dio();
    const String url = "${MyColors.baseUrl}/api/returns-product";
  ...
  Future deleteProduct(
    ...
    try {
      final response = await dio.delete("${MyColors.baseUrl}/api/returns-product/$productId",
          data: data,
      ...
      if (response.statusCode == 200) {
        Flushbar(
          message: "Barang retur berhasil dihapus",
      ...
      Flushbar(
        message: "Barang retur gagal dihapus",
        duration: const Duration(seconds: 3),
        backgroundColor: Colors.black, // Dark background
        flushbarPosition: FlushbarPosition.BOTTOM,
        borderRadius: BorderRadius.circular(10),
                                ...
                                GestureDetector(
                                  onTap: () {
                                    Navigator.push(
                                      context,
                                      MaterialPageRoute(
                                        builder: (context) => AddReturnScreen(id: widget.id, nama: widget.nama),
                                      ),
                                    );
                                  },
                    ...
                    child: FutureBuilder<List<ReturnModel>>(
                      ...
                      builder: (context, snapshot) {
                        if (snapshot.connectionState == ConnectionState.waiting) {
                          ...
                          return const Center(child: Text('No products retur available.', style: TextStyle(color: MyColors.whiteColor)));
                        } else {
                                ...
                                child: Container(
                                  ...
                                  child: ListTile(
                                    ...
                                    subtitle: Column(
                                      ...
                                      children: [
                                        Text('Nama Retur : ' + (product.namaRetur ?? 0).toString(), style: TextStyle(color: MyColors.whiteColor)),
                                        Text('Tanggal Retur : '+(product.createdAt ?? '').toString(), style: TextStyle(color: MyColors.whiteColor))
                                      ],
                                    ),
                                    trailing: Row(
                                        ...
                                        IconButton(
                                          ...
                                          onPressed: () {
                                            Navigator.push(
                                              context,
                                              MaterialPageRoute(
                                                builder: (context) => EditReturnScreen(id: product.id),
                                              ),
                                            );
                                          },
                                        ),

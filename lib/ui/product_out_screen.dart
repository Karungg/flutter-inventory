...
import 'package:inventory/ui/add_product_out_screen.dart';
import 'package:inventory/ui/edit_product_in_screen.dart';
import 'package:inventory/ui/edit_product_out_screen.dart';
...
class ProductOutScreen extends StatefulWidget {
  ...
  const ProductOutScreen({Key? key, this.id, this.nama}) : super(key: key);
...
class _ProductOutScreenState extends State<ProductOutScreen> {
  ...
  Future<List<ProductModel>> fetchProductData() async {
    final dio = Dio();
    const String url = "${MyColors.baseUrl}/api/products-out";
  ...
  Future deleteProduct(
    ...
    try {
      final response = await dio.delete("${MyColors.baseUrl}/api/products-out/$productId",
          data: data,
                                ...
                                GestureDetector(
                                  onTap: () {
                                    Navigator.push(
                                      context,
                                      MaterialPageRoute(
                                        builder: (context) => AddProductOutScreen(id: widget.id, nama: widget.nama),
                                      ),
                                    );
                                  },
                    ...
                    child: FutureBuilder<List<ProductModel>>(
                      ...
                      builder: (context, snapshot) {
                        if (snapshot.connectionState == ConnectionState.waiting) {
                          ...
                          return const Center(child: Text('No products out available.', style: TextStyle(color: MyColors.whiteColor)));
                        } else {
                                  ...
                                  child: ListTile(
                                    ...
                                    subtitle: Column(
                                      ...
                                      children: [
                                        ...
                                        Text('Tanggal Keluar : '+(product.createdAt ?? '').toString(), style: TextStyle(color: MyColors.whiteColor))
                                      ],
                                    ),
                                    trailing: Row(
                                        ...
                                        IconButton(
                                          ...
                                          onPressed: () {
                                            Navigator.push(
                                              context,
                                              MaterialPageRoute(
                                                builder: (context) => EditProductOutScreen(id: product.id),
                                              ),
                                            );
                                          },
                                        ),

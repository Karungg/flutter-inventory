...
class EditReturnScreen extends StatefulWidget {
  final int? id;
  const EditReturnScreen({Key? key, this.id}) : super(key: key);
...
class _EditReturnScreenState extends State<EditReturnScreen> {
  ...
  Future putProduct(
      String namaBarang, String namaRetur) async {
    var dio = Dio();
    ...
    try {
      final response = await dio.put("${MyColors.baseUrl}/api/returns-product/${widget.id}",
          data: data,
  ...
  Future<List<ReturnModel>> fetchProductData() async {
    final dio = Dio();
    String url = "${MyColors.baseUrl}/api/returns-product/${widget.id}";
  ...
  void initState() {
    ...
    namaRetur = TextEditingController();
    _getProductData();
  }

  Future<void> _getProductData() async {
    try {
      List<ReturnModel> productList = await fetchProductData();
      if (productList.isNotEmpty) {
        setState(() {
          namaBarang.text = productList[0].namaBarang ?? '';
          namaRetur.text = productList[0].namaRetur ?? '';
        });
      }
    } catch (e) {
  ...
  TextEditingController namaRetur = TextEditingController();
  @override
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              GestureDetector(
                onTap: () async {
                  if (namaBarang.text.length != 0 && namaRetur.text.length != 0) {
                    await putProduct(
                        namaBarang.text,
                        namaRetur.text)
                        .then((value) => {
                      if (value != null)
                        {
                          setState(() {
                            ...
                            Flushbar(
                              message: "Barang retur berhasil diupdate",
                    ...
                    Flushbar(
                      message: "Barang Retur harus diisi",
                      duration: const Duration(seconds: 3),
                      backgroundColor: Colors.black, // Dark background
                      flushbarPosition: FlushbarPosition.BOTTOM,
                      borderRadius: BorderRadius.circular(10),

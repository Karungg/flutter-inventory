...
class AddReturnScreen extends StatefulWidget {
  final String? id;
  final String? nama;
  const AddReturnScreen({Key? key,this.id, this.nama}) : super(key: key);
...
class _AddReturnScreenState extends State<AddReturnScreen> {
  ...
  Future postProduct(
    ...
    try {
      final response = await dio.post("${MyColors.baseUrl}/api/returns-product",
          data: data,
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              GestureDetector(
                onTap: () async {
                  if (namaBarangController.text.length != 0 && namaRetur.text.length != 0) {
                    await postProduct(

...
class EditProductInScreen extends StatefulWidget {
  final int? id;
  const EditProductInScreen({Key? key, this.id}) : super(key: key);

  @override
  State<EditProductInScreen> createState() => _EditProductInScreenState();
}

class _EditProductInScreenState extends State<EditProductInScreen> {
  ...
  Future putProduct(
    ...
    try {
      final response = await dio.put("${MyColors.baseUrl}/api/products-in/${widget.id}",
          data: data,
  ...
  Future<List<ProductModel>> fetchProductData() async {
    final dio = Dio();
    String url = "${MyColors.baseUrl}/api/products-in/${widget.id}";

...
class AddProductInScreen extends StatefulWidget {
  final String? id;
  final String? nama;
  const AddProductInScreen({Key? key,this.id, this.nama}) : super(key: key);

  @override
  State<AddProductInScreen> createState() => _AddProductInScreenState();
}

class _AddProductInScreenState extends State<AddProductInScreen> {
  ...
  Future postProduct(
    ...
    try {
      final response = await dio.post("${MyColors.baseUrl}/api/products-in",
          data: data,
              ...
              GestureDetector(
                onTap: () async {
                  if (namaBarangController.text.length != 0 && stokController.text.length != 0 && hargaController.text.length != 0) {
                    ...
                    Flushbar(
                      message: "Barang masuk harus diisi",
                      duration: const Duration(seconds: 3),
                      backgroundColor: Colors.black, // Dark background
                      flushbarPosition: FlushbarPosition.BOTTOM,
                      borderRadius: BorderRadius.circular(10),
